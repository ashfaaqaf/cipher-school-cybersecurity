...
The curriculum ships in the first bundle rather than being split per tab. It is the largest
single thing here — a third of the main chunk is lesson prose — but every lesson is
reachable from search, the quiz for a lesson is rendered inside the lesson, and the whole
thing is precached for offline use anyway, so splitting it would trade a smaller first load
for a spinner on the paths a reader actually walks.
