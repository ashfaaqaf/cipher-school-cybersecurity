...
const SKIP_DIRS = new Set(['audio', 'releases']);
...
/* Snapshot the entry page under a never-reused path. InfinityFree can retain
   the previous root response after an FTP upload and can ignore query strings,
   but it cannot return an older file for a path that did not previously exist. */
const releaseEntry = `releases/${version}.html`;
await mkdir(path.join(OUT, 'releases'), { recursive: true });
await writeFile(path.join(OUT, releaseEntry), await readFile(path.join(OUT, 'index.html')));
const urls = files.filter((file) => file !== 'index.html').map((file) => `${BASE}/${file}`);
...
const ENTRY = ${JSON.stringify(`${BASE}/${releaseEntry}`)};
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    ...
    caches.open(CACHE).then(async (cache) => {
      ...
      const entry = await fetch(ENTRY, { cache: 'reload', credentials: 'same-origin' });
      if (!entry.ok) throw new Error('Entry page failed with ' + entry.status);
      await cache.put(START, entry);
    }),
  );
});
