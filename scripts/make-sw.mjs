...
/* index.html is cached under START manually after a release-keyed network
   request. InfinityFree can briefly retain the previous `/` response after an
   FTP upload; a unique query prevents that stale HTML entering the new cache. */
...
const ENTRY = ${JSON.stringify(`${BASE}/?release=${version}`)};
